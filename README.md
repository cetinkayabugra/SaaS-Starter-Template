...
A floating chat widget (`components/chat/ChatWidget.tsx`) answers questions about the template, backed by Claude via `app/api/chat/route.ts`. It's optional — leave `ANTHROPIC_API_KEY` unset and the widget never renders and the endpoint returns 503.

**The endpoint is deliberately public** so the widget works on marketing pages for signed-out visitors. That means anyone who finds it can spend your API credits, so it ships with several guards:

- **Rate limit** — 10 requests/minute per IP (`lib/rate-limit.ts`).
- **Bounded input** — at most 20 messages per request, 2,000 characters each (`chatRequestSchema` in `lib/validations.ts`); only `user` and `assistant` roles are accepted, so a client can't inject a `system` turn.
- **Capped output** — `max_tokens` of 1024 and `effort: "low"`, since answers are short and chat is latency-sensitive.

⚠️ **Before deploying:** the rate limiter is in-memory, so it resets on redeploy and isn't shared across instances — on serverless the effective limit becomes (10 × instance count). Swap it for a shared store (Upstash Redis, Vercel KV) if you run more than one instance. Consider also gating the route behind `auth()` if you'd rather only signed-in users can spend tokens.
...
│   ├── chat/       # Claude-backed chat endpoint (streaming)
...
lib/                # auth config, Prisma client, Stripe client, plans, env validation, analytics, GitHub stars fetch, rate limiting
